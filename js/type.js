// 프로그래밍 언어에서 다루는 모든 데이터는 각자의 형태를 지닌다. 이를 타입이라고 한다.
...
//2. 템플릿 리터럴
// - 변수와 문자열을 함께 사용할 때 es6 버전부터는 +기호 없이 편리하게 조합할 수 있는 방식을 제공한다. 이를 템플릿 리터럴이라고 한다.
...
// 숫자형 문자는 + 의 경우 문자 연산이 되지만(조합 가능) 나머지 연산은 엔진이 숫자로 임의 변경하여 연산한다. 이를 암묵적 형변환이라 한다.
...
// 5. Boolean(=조건분개, 조건문, 비교문에서 많이 쓰임 - 원시타입)
// 컴퓨터는 모든 데이터를 0과 1로 읽는다. 이를 이용하여 true와 false(리턴값)를 나타내는 방식을 표현하는데 이를 boolean이라 한다.
...
// undefined와 같이 비어있는 값이라는 의미는 가진다.
console.log(undefined == null); //== : 값이 같음
// 하지만 데이터의 타입은 undefined와 다르다.
console.log(undefined === null); //=== : 값도 같고 type도 같음
