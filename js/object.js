...
// signiture : {key1 : value1, key2 : value2...} -공통된 주제로 만들어야함
const car = {
  maker: "hyundai",
  model: "avante",
  color: "white",
...
console.log(car.maker); //잘 사용하는 형식의 문구
...
const pet1 = {
  ...
  eat: function (food) {
    ...
    // console.log(`${this.name}는 ${food}을 좋아합니다.`);
  },
};
...
const timer = setInterval(function () {
  ...
  const min = now.getMinutes();
  const sec = now.getSeconds();
  const timeTag = `<span>${hours}</span> : <span>${min}</span> : <span>${sec}</span>`;
  //console.log(hours, min, sec);
