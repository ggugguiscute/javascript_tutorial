...
// 함수는 다른 함수의 리턴 값 또는 파라미터 변수값으로 사용될때 이를 callback function이라 한다.
//>>>const func = () = {}; //값
//>>>const abc = if(true){} else{}; //값이 아님
...
// 3. 콜백 지옥
// 여러 기능을 분리하여 각각 다른 함수에 정의하고, 이들을 일정 시간 또는 일정 수행 간격으로 연쇄적 실행을 할 경우 콜백함수를 반복해서 사용하게 된다. 이들은 모두 들여쓰기로 표시되고, 들여쓰기가 감당하기 어려울 정도로 깊어지는 현상을 콜백지옥이라 한다.(ㅋㅋ)
function getData(a, b, cb) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      console.log("show getData function data:" + a);
      resolve({ a: a, b: b });
    }, 1000);
  });
}
...
// async가 붙은 함수는 반드시 프라미스를 반환하고, 프라미스가 아닌 것은 프라미스로 감싸 반환한다.(자동)
