...
// - signiture : array.forEach((value, index, array)(**콜백함수) => {})
...
// 4. reduce - 누적하여 더해줌(합산메서드) << 프론트에서는 잘 사용 안하기 때문에 천천히 이해해도 됨
// - signiture : array.reduce((accumulator(누적), currentValue(현재값), currentIndex(현재인덱스), arry(배열)) => {})
...
const reduceNumber = reduceArr.reduce((accumulator, currentValue) => {
  ...
  //0, 1, 2까지 누적한 합산값 3
  //0, 1, 2, 3까지 누적한 합산값 6
  //0, 1, 2, 3, 4까지 누적한 합산값 10. 따라서 결과는 0, 1, 3, 6, 10
  console.log(currentValue);
  //두번째 값부터 표시된다.
  //0, 1까지의 현재값(두번째값)) 1
  //0, 1, 2까지의 현재값 2
  //0, 1, 2, 3까지의 현재값 3
  //0, 1, 2, 3, 4까지의 현재값 4. 따라서 결과는 1, 2, 3, 4, 10
